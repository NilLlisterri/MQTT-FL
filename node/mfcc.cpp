...
public:
    ...
    int processFile (std::ifstream &wavFp, std::ofstream &mfcFp) {
        // Read the wav header    
        wavHeader hdr;
        ...
        while (wavFp.gcount() == bufferLength*bufferBPS && !wavFp.eof()) {
            v_d frameResult = processFrame(buffer, bufferLength);
            mfcFp << v_d_to_string(frameResult);
            wavFp.read((char *) buffer, bufferLength*bufferBPS);
        }
        delete [] buffer;
        buffer = nullptr;
    ...
    v_d processSample(int16_t* sample, int size) {
        // Initialise buffer
        uint16_t bufferLength = winLengthSamples-frameShiftSamples;
        int bufferBPS = (sizeof sample[0]);
        int sampleIterator = 0;
        
        // Read and set the initial samples
        // wavFp.read((char *) buffer, bufferLength*bufferBPS);
        for (int i = 0; i < bufferLength; i++)
            prevsamples[i] = sample[i];

        sampleIterator += bufferLength;
        // Recalculate buffer size
        bufferLength = frameShiftSamples;
        int16_t* buffer = new int16_t[bufferLength];
        ...
        // Read data and process each frame
        v_d result;
        while (sampleIterator <= size) {
            v_d frameResult = processFrame(buffer, bufferLength);
            result.insert(result.end(), frameResult.begin(), frameResult.end());
            // wavFp.read((char *) buffer, bufferLength*bufferBPS);
            for (int i = 0; i < bufferLength*bufferBPS; i++) {
                buffer[i] = sample[sampleIterator + i];
            }
            sampleIterator += bufferLength*bufferBPS;
        }
        delete [] buffer;
        buffer = nullptr;
        return result;
